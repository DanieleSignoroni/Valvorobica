 ...
 * 
 * @author Andrea Gatta 01/12/2022 <br>
 *         </br>
 *         <b>70877 23/01/2023 DSSOF3</b> <br>
 *         Commentata la riga dove setto la STRINGA_RIS_UTE_1 al valore del json
 *         di entrata. <br>
 *         Questo perche' l'articolo cliente che ci passano non equivale a
 *         quello codificato in panthera.<br>
 *         <b>70880 25/01/2023 DSSOF3</b> <br>
 *         CondizioniDiVendita sulle righe documento.<br>
 *         <b>70958 20/02/2023 DSSOF3</b> <br>
 *         UM presa dall'articolo pth e non dal json.<br>
 *         <b>70979 01/03/2023 DSSOF3</b> <br>
 *         Introdotta la gestione del saldo cliente e non piu il saldo base.<br>
 *         <b>71003 17/03/2023 DSSOF3</b> <br>
 *         Nella creazione doc ven non setto piu numRifInt e dataRifInt ma
 *         numOrdCLI e dataOrdCLI. <br>
 *         Settate alle righe anche le provvigioni.<br>
 *         <b>71037 03/04/2023 DSSOF3</b> <br>
 *         Correzione - con la fix precedente e' stato cambiato il set dei
 *         riferimenti, ma non e' stato aggiornato il modo in cui viene updatato
 *         il parametro contenente l'ultima data ok di processo.<br>
 *         Visto che prima era valorizzata con la data rif int, ora e' stata
 *         cambiata con la data ord int. (cioe' quella dell'ordine cliente.)<br>
 *         <b>71066 03/05/2023 AGSOF3</b> <br>
 *         Modifica al reperimento in funzione della data, adesso vengono
 *         processate tutte le date a partire da oggi - 15gg fino ad oggi,
 *         scartando però tutti i documenti che esistono già, facendo il
 *         controllo con la chiave id_ddt<br>
 *         <b>71067 04/05/2023 DSSOF3</b> <br>
 *         Nella ricerca delle condizioni vendita
 *         {@link #recuperaCondizioniDiVendita(DocumentoVendita, DocumentoVenRigaPrm, String)}<br>
 *         assumo che la quantita' magazzino passata alla classe che recupera le
 *         condizioni sia uguale a {@link BigDecimal@#ONE} .<br>
 *         <b>71119 06/06/2023 DSSOF3</b> <br>
 *         Nella lettura, creo un file di log di supporto per capire quali
 *         documenti vanno in errore.<br>
 *         <b>71119 07/06/2023 AGSOF3</b> <br>
 *         Non leggo.<br>
 *         <b>71148 21/06/2023 DSSOF3</b> <br>
 *         Convalido il doc anche se ho creato il lotto generico<br>
 *         <b>71156 28/06/2023 DSSOF3</b> <br>
 *         Sistemare la generazione dei lotti quando creo la riga documento
 *         vendita.<br>
 *         <b>71225 19/09/2023 DSSOF3</b> <br>
 *         Aggiunto metodo {@link #lanciaCovalidaNonRiusciti(Map, PrintWriter)}
 *         per poter ri-lanciare la convalida di tutti quei documenti che non
 *         sono stati convalidati al primo giro.</br>
 *         <b>71356 DSSOF3 18/12/2023</b>
 *         <p>
 *         Nel metodo {@link #esisteGiaDocVen(String)}, migliorare la ricerca di
 *         quei documenti gia' presenti.<br>
 *         Specificando che deve avvenire solo su documenti con
 *         {@value DocumentoVenditaTM#R_CAU_DOC_VEN} = 'VDP'.
 *         </p>
 */
...
public class YGetDDTContoDep extends GenRequestJSON {
	...
	protected Map execute(Map m) {
		...
		// 71119 FIne
		try (FileWriter fileWriter = new FileWriter(log.getAbsolutePath(), true);
				PrintWriter printWriter = new PrintWriter(fileWriter)) {
			// 71066 ini remmata
			// String ultimaDataOkStringa =
			// YUtilsPortal.getParametroPersonalizzazione("YValvorobicaContoDeposito",
			// "UltimaDataProcessataDDT").getValore();
			// java.util.Date ultimaDataOk = new
			// SimpleDateFormat("yyyy-MM-dd").parse(ultimaDataOkStringa);
			// ultimaDataOk = TimeUtils.addDays(ultimaDataOk, 1);
			// 71066 fine remmata
			printWriter.println("INIZIO RICEZIONE DATI CONTO DEPOSITO " + TimeUtils.getCurrentTimestamp().toString()
					+ "------------------------->"); // 71119
			java.util.Date oggi = TimeUtils.getCurrentDateTime();
			java.util.Date ultimaDataOk = TimeUtils.addDays(oggi, -15);// 71066 la data da cui partire a processare è
			// oggi - 15gg
			...
			while (ultimaDataOk.before(oggi)) {// processo tutti i giorni dall'ultima data ok fino ad oggi compreso
				...
				Map<String, String> res = YUtilsPortal.chiamaWsEsterno(endpointGetDDT, methodRequest, reqParams, true);
					...
					while (iterTestate.hasNext()) {
						...
						if (testata.Dettagli.size() > 0) {// se non ho almeno una riga non faccio niente
							...
							if (docven != null) { // 71119
								...
								if (docven != null) {
									// Se ho creato il lotto generico evito la convalida dato che andrebbe in errore
									// per i saldi
			...
			printWriter.println("<----------------------------- TERMINATA RICEZIONE DATI CONTO DEPOSITO "
					+ TimeUtils.getCurrentTimestamp().toString()); // 71119
			printWriter.close();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	/**
	 * <h1>Lancia convalida per i documenti non convalidati</h1> <br>
	 * Daniele Signoroni 19/09/2023 <br>
	 * </br>
	 * <b>71225 DSSOF3 19/09/2023</b>
	 * <p>
	 * Capitava spesso che la procedura non convalidasse alcuni documenti che a mano
	 * venivano convalidati senza cambiare dati.<br>
	 ...
	 */
	@SuppressWarnings("rawtypes")
	protected void lanciaCovalidaNonRiusciti(Map<String, YDocumentoVendita> docsNonConvalidati,
				...
				if (docVen.save() >= 0) { // salvo ma non committo così ho la chiave e posso aggiungere le righe
					...
					while (iterRighe.hasNext()) {
						...
						if (rc < 0) {
							...
							if (rc == -1) {
								errori.add("Problema nel salvataggio riga art: " + dettaglio.getCOD_Articolo() + " ("
										+ msg + ") per la testata: " + testata.getID_DDTTestata());
							} else if (rc == -2) {
								msg = " lotto vuoto ";
								errori.add("Problema nel salvataggio riga art : " + dettaglio.getCOD_Articolo() + " ("
										+ msg + ") per la testata : " + testata.getID_DDTTestata());
					...
					// YUtilsPortal.updateValoreParametro(docVen.getDataRifIntestatario().toString(),
					// "YValvorobicaContoDeposito", "UltimaDataProcessataDDT"); 71037
					YUtilsPortal.updateValoreParametro(docVen.getDataOrdineIntestatario().toString(),
							"YValvorobicaContoDeposito", "UltimaDataProcessataDDT");
				}
			}
		} catch (Exception e) {
	 ...
	 * 
	 * @param id_DDTTestata
	 * @return true se esiste già un documento con id_DDTTestata nel nro ordine
	 *         intestatario
	 */
	protected boolean esisteGiaDocVen(String id_DDTTestata) {
		...
		try {
			String select = "SELECT 1 FROM " + DocumentoVenditaTM.TABLE_NAME + " " + "WHERE "
					+ DocumentoVenditaTM.ID_AZIENDA + " = '" + Azienda.getAziendaCorrente() + "' " + "AND "
					+ DocumentoVenditaTM.NUM_ORD_CLIENTE + " = '" + id_DDTTestata + "' ";
			// aggiunto 71356 dssof3
	...
	@SuppressWarnings("unchecked")
	protected int creaDocVenRigDaDettaglioJson(DocumentoVendita docVen, YContoDepDDTDettaglio dettaglio)
			throws SQLException {
		int rc = -1;
		...
		if (articolo != null) {
			// docVenRig.setAlfanumRiservatoUtente1(dettaglio.getCOD_ArticoloCliente());
			// DSSOF3 remmato perche' l'articolo che ci arriva non e' quello codificato in
			// panthera.
			...
			if (docVenRig.getArticolo() != null) {
				CondizioniDiVendita cdv = recuperaCondizioniDiVendita(docVen, docVenRig,
						/* UMPth */articolo.getIdUMRiferimento());
				if (cdv != null) {
	...
	/**
	 * <b>71156 28/06/2023 DSSOF3</b>
	 * <p>
	 * Ridisegnato, ordino per QTA_GIAC_PRM DESC in modo da prendere prima quelli
	 * con piu giacenza,<br>
	 * In seguito sistemata la logica.
	 * </p>
	 * 
	 * @param idArticolo
	 * @param docVenRig
	 */
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static void sistemaLotti(String idArticolo, DocumentoVenRigaPrm docVenRig) {
		String where = " " + SaldoMagLottoClienteTM.ID_AZIENDA + " = '" + Azienda.getAziendaCorrente() + "' " + " AND "
				+ SaldoMagLottoClienteTM.ID_ARTICOLO + " = '" + docVenRig.getIdArticolo() + "' " + " AND "
				+ SaldoMagLottoClienteTM.ID_MAGAZZINO + " = '" + docVenRig.getIdMagazzino() + "' " + " AND "
				+ SaldoMagLottoClienteTM.ID_CONFIG + " = '"
				+ (docVenRig.getIdConfigurazione() != null ? docVenRig.getIdConfigurazione() : 0) + "'" + " AND "
				+ SaldoMagLottoClienteTM.ID_CLIENTE + " = '"
				+ ((YDocumentoVendita) docVenRig.getTestata()).getIdCliente() + "' " + " AND "
				+ SaldoMagLottoClienteTM.ID_LOTTO + " != 'LOTTO_GEN' " + " AND " + SaldoMagLottoClienteTM.ID_LOTTO
				+ " <> '-' ";
		...
		try {
			...
			if (saldiLotti.size() > 0) {
				...
				while (iterLotti.hasNext()) {
					...
					if (quantitaLotto.compareTo(BigDecimal.ZERO) == 0 || quantitaLotto.compareTo(BigDecimal.ZERO) < 0)
						continue;
	...
	@SuppressWarnings("rawtypes")
	public static Lotto getLottoGenerico(String idArticolo) {
		Lotto lotto = null;
		String where = LottoTM.ID_AZIENDA + "='" + Azienda.getAziendaCorrente() + "'" + "AND  " + LottoTM.ID_ARTICOLO
				+ "= '" + idArticolo + "'" + " AND " + LottoTM.ID_LOTTO + " = 'LOTTO_GEN' ";
		Vector lotti;
		try {
	...
	@SuppressWarnings("rawtypes")
	public static Lotto getLottoWithIdLottoArticolo(String idArticolo, String idLotto) {
		String where = LottoTM.ID_AZIENDA + "='" + Azienda.getAziendaCorrente() + "'" + "AND  " + LottoTM.ID_ARTICOLO
				+ "= '" + idArticolo + "'" + "AND  " + LottoTM.ID_LOTTO + "= '" + idLotto + "'";
		try {
	...
	public static Lotto creaLotto(BigDecimal qta, String codiceArticolo, String codiceLotto) {
		Lotto lotto = null;
		try {
	...
	public static CondizioniDiVendita recuperaCondizioniDiVendita(DocumentoVendita docVen, DocumentoVenRigaPrm docVenRig,
			String uMPth) {
		...
		try {
			...
			cdv = rcdv.ricercaCondizioniDiVendita(// recupero le condizioni di vendita per avere il prezzo del
					// contenitore
	...
	public static Articolo getArticolo(String cod_Articolo) {
		Articolo art = null;
		try {

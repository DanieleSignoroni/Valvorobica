 ...
 *  <b>71660 DSSOF3	10/16/2024</b>  <p>Attivazione catalogo</p>
 *  <b>72041 DSSOF3 10/07/2025</b>	<p>Parametro login per gestire spinner</p>
 */
...
public class YValidatePortalLogin extends HttpServlet {
	...
	protected void processAction(HttpServletRequest req, HttpServletResponse resp) {
		try {
			...
			if(chiaveUser[4] != null) {
				if(company.equals("001")) { 
					if(userPortal.isECommerceEnabled(company))
						userPortal.isCatalogoAttivo = true;
					if(userPortal.isContoDepositoEnabled(company))
						userPortal.setContoDepositoAttivo(true);
				}
			}
			...
			try {
				...
				if(company.equals("001")) {
					tokenUID = ParametroPsn.getValoreParametroPsn("YUserPortalToken", "TokenUID");
				}else if(company.equals("002")){
					tokenUID = ParametroPsn.getValoreParametroPsn("YUserPortalToken_002", "TokenUID");
				}
				resp.setHeader("authorization", tokenUID);
				userPortal.setTokecUID(tokenUID);

				req.getSession().setAttribute("YUserPortal", userPortal);
				req.getSession().setAttribute("YUserPortalLogin", true); //72041
